...
DEFAULT_MANIFEST = Path("docs/p6/public_core_export_manifest.json")
DEFAULT_SIX_REPO_MANIFEST = Path("docs/p6/six_repo_manifest.json")
DEFAULT_OUT_ROOT = Path(".tmp/p6_m2_derivation")
OUTPUT_JSON = "public_export_plan.json"
OUTPUT_MD = "public_export_plan.md"
...
class ExportItemDict(TypedDict):
    ...
    stability: str
    allowed_for_public: bool
    commercial_logic_detected: bool
...
def _path_status(root: Path, declared_paths: list[str]) -> tuple[PathStatus, list[str]]:
    ...
    for path_text in declared_paths:
        if _is_placeholder_path(path_text):
            placeholder_count += 1
            notes.append(f"{path_text}: draft placeholder, not a completed export path.")
            continue
...
def _commercial_logic_detected(parts: list[str]) -> bool:
    ...
    if any(marker in lowered for marker in allowed_negative_contexts):
        return False
    markers = (
        "payment",
        "billing",
...
def _secret_risk_detected(parts: list[str]) -> bool:
    lowered = " ".join(parts).lower()
    markers = ("secret", "api key", "password", "token", "credential", "db credentials")
    return any(marker in lowered for marker in markers)
...
def build_report(
    *,
    ...
    allowed_consumers = {"Zephyr-base", "Zephyr-Web", "Zephyr-site"}
    ...
    for item_obj in _as_list(manifest["allowed_exports"]):
        ...
        declared_paths = [_as_str(entry) for entry in _as_list(item["paths"])]
        ...
        commercial_logic = _commercial_logic_detected(scan_parts)
        secret_risk = _secret_risk_detected(scan_parts)
        allowed_for_public = not commercial_logic and not secret_risk
        if not allowed_for_public:
            active_blockers += 1
        export_items.append(
            {
                ...
                "allowed_for_public": allowed_for_public,
                "commercial_logic_detected": commercial_logic,
                "secret_risk_detected": secret_risk,
                "notes": path_notes,
            }
        )
    forbidden_exports = [_as_str(item) for item in _as_list(manifest["forbidden_exports"])]
    commercial_forbidden = any(_commercial_logic_detected([item]) for item in forbidden_exports)
    private_core_forbidden = any(
        marker in " ".join(forbidden_exports).lower()
        for marker in ("private connector", "web-core", "private runtime", "private core")
    )
    secret_forbidden = any(_secret_risk_detected([item]) for item in forbidden_exports)
    issues: list[str] = []
    if consumer_issues:
        issues.extend([f"Unexpected public target consumer: {item}" for item in consumer_issues])
    ...
    notes = [
        "Public export is dry-run only and does not publish or copy code.",
        "Placeholder paths are intentional draft markers, not completed exports.",
        "Actual export/scaffold mechanics remain a later P6 step.",
    ]
    ...
    return {
        "schema_version": 1,
        "report_id": "zephyr.p6.m2.public_export_dry_run.v1",
        ...
        "source": {
            ...
            "zephyr_dev_head_sha": _best_effort_git_sha(root),
        ...
        "forbidden_export_check": {
            ...
            "forbidden_export_terms_cover_commercial_logic": commercial_forbidden,
            "forbidden_export_terms_cover_private_core": private_core_forbidden,
            "forbidden_export_terms_cover_secret_risk": secret_forbidden,
        },
        "requires_m2_or_later_mechanics": True,
        "issues": issues,
...
def render_markdown(report: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "- public export is a dry-run plan",
        "- public export does not mean publishing the whole Zephyr-dev repo",
        "- unresolved placeholder paths are intentional draft markers",
        "- later P6 work is required before any real export or scaffold happens",
        "",
        "## Export items",
    ]
    for item in items:
        lines.append(
            ...
            f"path_status={item['path_status']} | allowed_for_public={item['allowed_for_public']}"
        )
    lines.extend(["", "## Notes"])
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build a public core export dry-run plan.")
